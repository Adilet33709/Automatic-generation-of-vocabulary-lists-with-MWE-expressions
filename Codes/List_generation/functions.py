...
## Function that returns list of dictionaries of MWEs of EFLlex with or without dispersion
def get_EFLlex(disp):
    EFLlex_list = pd.read_excel("Resources/EFLlex.xlsx", header=None).iloc[:,0].tolist()
    bigrams = get_dictionary("EFLlex_bigram_freqs_OSEWB", disp)
    trigrams = get_dictionary("EFLlex_trigram_freqs_OSEWB", disp)
    EFLlex_dicts = []
    for word in EFLlex_list:
        new_dict = {}
        new_dict["word"] = str(word)
        word = str(word)
        if word in bigrams.keys():
            new_dict["score"] = bigrams[word]
        elif word in trigrams.keys():
            new_dict["score"] = trigrams[word]
        else:
            new_dict["score"] = 0
        EFLlex_dicts.append(new_dict)
    return EFLlex_dicts
...
def get_Pickard(disp, comp):
    assert 0 <= comp <= 100
    auto_list_full = pd.read_excel("Resources/Pickard.xlsx", header=None).iloc[:,0].tolist()
...
def get_auto_list(disp, auto_list,comp= 100):
    list_single = get_single_lemmas(disp)
    if auto_list == "EFLlex":
        auto_dicts = get_EFLlex(disp)
    elif auto_list == "Pickard":
